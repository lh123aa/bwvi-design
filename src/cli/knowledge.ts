import { loadChunk, listChunks, getChunkDescription, reloadKnowledge } from "../knowledge/loader.js";
...
export async function knowledgeCommand(args: string[]) {
  ...
  if (sub === "reload") {
    ...
    console.log(JSON.stringify({ status: "ok", chunks_loaded: count, source: "knowledge/*.md" }, null, 2));
    return;
  }
  ...
  if (sub === "list" || !sub) {
    ...
    const list = chunks.map((id) => ({
      id,
      description: getChunkDescription(id),
